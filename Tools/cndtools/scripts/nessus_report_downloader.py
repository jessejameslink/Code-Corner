    ...
    for scan_id in scan_id_list:
        ...
        if json.loads(resp2.text)["status"] == "ready":
            ...
            if checkStatus(resp3, "Started downloading the nessus report",
                ...
                global filename0
                filename0 = filename.partition('_')[0]
                try:
                    ...
                    filename2=filename0.split(".")[0]+'_'+json_user_data["chapters"]+'.'+filename.split(".")[-1]
                    nessus_file = open(filename2, "wb")
                    nessus_file.write(resp3.content)
                    nessus_file.close()
                    printMessage("Report was saved in "  + os.getcwd() + "/" + filename2, 1)
                    cwd = os.listdir('.')
                    shutil.move(os.getcwd() + "/" + filename2,path)
                    printMessage("Report Moved to" + path)
                    os.remove(filename)
                    printMessage("Temporary pdf deleted." + filename)
                    printMessage("\n", 99)
...
def main():
    ...
    global path
    path = '/root/working/nessus'
    print path

    #create nessus folder
    try:
        os.mkdir( path, 0755 );
    except OSError:
        print ("Creation of the directory %s failed" % path)
    else:
        print ("Nessus Directory Created %s " % path)    
    # Nessus server url
    if ":" in args.server:
