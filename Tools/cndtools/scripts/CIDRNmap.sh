...
scantypemenu () {
	...
	if [ "$scantypemenuinput" = "1" ]; then
		...
		seq 0 255 | xargs -P 10 -I{} nmap --top-ports 2000 --open -sT -Pn -T4 --min-hostgroup 256 -min-parallelism 64 -oG /root/working/external/nmap/"$orgnameinput"_nmap_open.txt --append-output "$cidroutput"
		
		# Grep for open ports and export IPs to a file
		grep "/open/" /root/working/external/nmap/"$orgnameinput"_nmap_open.txt   | cut -d" " -f 2 | sort -u  > /root/working/external/nmap/"$orgnameinput"_ips.txt
